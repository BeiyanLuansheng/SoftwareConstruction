...
public class LowestPrice {
	...
	public int shopping(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
		...
		for (List<Integer> s : special) {
			...
			for (j = 0; j < needs.size(); j++) { //j的范围是从 0到needs.size()-1
				int diff = clone.get(j) - s.get(j);
				if (diff < 0) //差小于零表示这个方案不可接受，去掉‘=’
					break; //直接下一个方案，改continue为break
				clone.set(j, diff);
			}
			if (j == needs.size()) {
				// s中的最后一个数是此方案的花费，由于s.size() = needs.size() + 1，故最后一个数是j而非j-1
	...
	public int dot(List<Integer> a, List<Integer> b) {//求最高花费
		int sum = 0;
