...
public class EventManager {
	...
	public static int book(int day, int start, int end) {
		//所设置的key高三位表示day，低两位表示hour
		if(temp.containsKey(day*100+start)) //先判断key在Map中，在则value+1
			temp.put(day*100+start, temp.get(day*100+start)+1);
		else
			temp.put(day*100+start, 1); //不在则设置初值为1
		if(temp.containsKey(day*100+end)) //先判断key在Map中，在则value-1
			temp.put(day*100+end, temp.get(day*100+end)-1);
		else
			temp.put(day*100+end, -1);//不在则设置初值为-1
